        ...
        if getn(distribution) > 0 then
            if settings.disenchant_distribution then
                ...
                for i = 1, getn(distribution) do
					...
					tooltip:AddLine(format('         %s  ', money.to_string2(history.market_value((event.item_id or 0) .. ':' .. 0) or history.value((event.item_id or 0) .. ':' .. 0))), color.tooltip.disenchant.distribution())
                end
            end
