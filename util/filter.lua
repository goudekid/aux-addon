...
M.filters = {
	...
	['bid-difference'] = { -- difference between bid and buyout
		input_type = 'money',
        validator = function(amount)
            return function(auction_record)
                return (auction_record.buyout_price - auction_record.bid_price) >= amount
            end
        end
    },
	...
	['bid-diff-percent'] = {
        ...
        validator = function(pct)
            return function(auction_record)
                return (auction_record.buyout_price / auction_record.bid_price) * 100 >= pct
            end
        end
    },
